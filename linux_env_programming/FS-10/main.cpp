...
void handle_data(int size, int src, int dest, char BUF[buf_size], int& data_bytes, int& copied_bytes)
{
    ...
    while (size > 0)
    {
        ...
        if (bytes_read < 0) handle_error("Error reading from source file");
        if (bytes_read == 0) break;
        bytes_written = write(dest, BUF, bytes_read);
        if (bytes_written < 0) handle_error("Error writing to destination file");
...
{
    if (argc != 3) handle_error("Invalid arguments. Usage: ./copy source-file destination-file");

    int fd_src = open(argv[1], O_RDONLY);
    if (fd_src == -1) handle_error("Error opening source file");

    int fd_dst = open(argv[2], O_WRONLY | O_TRUNC | O_CREAT, 0644);
    if (fd_dst == -1) handle_error("Error opening destination file");
    ...
    close(fd_src);
    close(fd_dst);
    std::cout << "Successfully copied " << copied_bytes << " bytes (data: " << data_bytes << ", hole: " << hole_bytes
              << ")." << std::endl;
    return 0;
}
